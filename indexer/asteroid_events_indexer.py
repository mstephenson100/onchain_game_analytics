...
import db.asteroid_db as asteroid_db
...
mongo_table = phase + "-asteroid-events"
...
class DispatcherIndexer(StarkNetIndexer):
    ...
    def initial_configuration(self) -> Filter:
        ...
        return IndexerConfiguration(
            ...
            .add_event(
                EventFilter()
                .with_from_address(asteroid_address)
                .with_keys([asteroid_transfer_key])
            )
            .add_event(
                EventFilter()
                .with_from_address(asteroid_address)
    ...
    async def handle_data(self, info: Info, data: Block):
        ...
        for event_with_tx in data.events:
            ...
            if event_hex == asteroid_transfer_key:
                await processAsteroidTransfer(tx_hash, event, block_number, fee, timestamp)
        ...
        if len(txns) > 0:
            txn_db.txnCounter(block_number, len(txns), timestamp, "asteroid_txns_per_block")
...
async def processAsteroidTransfer(tx_hash, event, block_number, fee, timestamp):

    event_type = "AsteroidTransfer"
    from_address = felt.to_hex(event.data[0])
    to_address = felt.to_hex(event.data[1])
    asteroid_id = felt.to_int(event.data[2])
    print("block_number: %s, event_type: %s, txn_id: %s, from_addr: %s, asteroid_id: %s, to_addr: %s" % (block_number, event_type, tx_hash, from_address, asteroid_id, to_address))
    asteroid_db.asteroidTransfer(tx_hash, block_number, asteroid_id, from_address, to_address, fee, timestamp)
    txn_db.feeLogger(tx_hash, block_number, fee, timestamp, "asteroid")


async def processBridgedFromL1(tx_hash, event, block_number, from_address, fee, timestamp):

    event_type = "BridgedFromL1"
    asteroid_id = felt.to_int(event.data[0])
    to_address = felt.to_hex(event.data[2])
    print("block_number: %s, event_type: %s, txn_id: %s, from_addr: %s, asteroid_id: %s, to_addr: %s" % (block_number, event_type, tx_hash, from_address, asteroid_id, to_address))
    grab.snagAsteroid(asteroid_id, asteroids_storage, network)
    asteroid_db.bridgedFromL1(tx_hash, block_number, from_address, asteroid_id, to_address, fee, timestamp)
    txn_db.feeLogger(tx_hash, block_number, fee, timestamp, "asteroid")
...
async def processBridgedToL1(tx_hash, event, block_number, from_address, fee, timestamp):
    ...
    print("block_number: %s, event_type: %s, txn_id: %s, from_addr: %s, asteroid_id: %s, eth_address: %s" % (block_number, event_type, tx_hash, from_address, asteroid_id, eth_address))
    asteroid_db.bridgedToL1(tx_hash, block_number, from_address, asteroid_id, eth_address, fee, timestamp)
    txn_db.feeLogger(tx_hash, block_number, fee, timestamp, "asteroid")
...
async def main():
    ...
    global network
    global asteroid_address
    global asteroid_transfer_key
    ...
    if os.path.exists(config_file):
        ...
        asteroid_address = felt.from_hex(config.get('contracts', 'asteroid_address'))
        asteroid_transfer_key = felt.from_hex(config.get('selectors', 'transfer_key'))
